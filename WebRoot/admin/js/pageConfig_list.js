/*列表数据*/
const LIST = getAminUrl('admin/CENTER/PAGECONFIG/LIST')
/*列表删除*/
const DELETE = getAminUrl('admin/CENTER/PAGECONFIG/DELETE');
/*编辑*/
const EDIT = getAminUrl('admin/CENTER/PAGECONFIG/EDIT');
/*批量删除*/
const BATCH_DELETE = getAminUrl('admin/CENTER/PAGECONFIG/BATCH/DELETE');
/*查找单个对象*/
const GET = getAminUrl('admin/CENTER/PAGECONFIG/GET');
//行对象
var rowObj = "";
...
/*删除*/
function del(obj) {
	var pageConfigId = obj.data.pageConfigId;
...
function edit(obj) {
	...
	if(obj){
		var pageConfigId = obj.data.pageConfigId;
		url = EDIT + "?pageConfigId=" + pageConfigId;
		 title = '页面配置';
	}	
...
function batchDel() {
	...
	layer.confirm('确认要删除吗？', function(index) {
		...
		$.each(selectData,function(i,e){
			array.push(e.pageConfigId);
		 })
		reqPostHasParameter(BATCH_DELETE, {"pageConfigIdArr":array},function(result) {
...
function updateRowData(obj){
	var reqData = obj.field;
	 reqPostHasParameter(GET, {"pageConfigId":reqData.pageConfigId}, function(result) {

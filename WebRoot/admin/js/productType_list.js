/*列表数据*/
const LIST = getAminUrl('admin/CENTER/PRODUCTTYPE/LIST')
/*列表删除*/
const DELETE = getAminUrl('admin/CENTER/PRODUCTTYPE/DELETE');
/*编辑*/
const EDIT = getAminUrl('admin/CENTER/PRODUCTTYPE/EDIT');
/*批量删除*/
const BATCH_DELETE = getAminUrl('admin/CENTER/PRODUCTTYPE/BATCH/DELETE');
/*查找单个对象*/
const GET = getAminUrl('admin/CENTER/PRODUCTTYPE/GET');
//行对象
var rowObj = "";
...
function edit(obj) {
	...
	if(obj){
		var productTypeId = obj.data.productTypeId;
		url = EDIT + "?productTypeId=" + productTypeId;
		 title = '产品类别';
	}	
...
function batchDel() {
	...
	layer.confirm('确认要删除吗？', function(index) {
		...
		$.each(selectData,function(i,e){
			array.push(e.productTypeId);
		 })
		reqPostHasParameter(BATCH_DELETE, {"productTypeIdArr":array},function(result) {
...
function updateRowData(obj){
	var reqData = obj.field;
	 reqPostHasParameter(GET, {"productTypeId":reqData.productTypeId}, function(result) {

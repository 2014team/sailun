/*更新*/
const UPDATE = getAminUrl('admin/CENTER/PRODUCT/UPDATE');
/*保存*/
...
layui.use([ 'form', 'layer','upload','layedit' ], function() {
	...
	//保存
		 form.on('submit(editSave)', function(obj) {
			...
			formData.append("productId", reqData.productId);
...
//检查是否保存还是修改操作
function checkSave() {
	var userId = $("#productId").val();
	return userId;
};

//回显Select选值
function echoSelect(){
	echoSelectData("productTypeId",$("#productTypeId").attr('value'));
